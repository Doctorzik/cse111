...
def test_suffix():
    """Verify that the sufix function works corectly.
    Parameters: none
    Return: nothing
    """
    # Call the sufix function and verify that it returns a string.
    suf = suffix("question", "station")

    assert isinstance(suf, str), "sufix fuction must return a string"
    # Call the sufix function ten times and use an assert
    # statement to verify that the string returned by the
    # sufix function is correct each time.
    assert suffix("", "") == ""
